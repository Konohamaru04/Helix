...
## Chat transcripts stream lightweight metadata and lazy-load heavy artifacts

Large tool traces and RAG source excerpts caused the renderer to retain and render too much per-turn state while long-running chats were still active. The bridge now persists assistant text and metadata incrementally in SQLite, stream events carry counts instead of the full tool/source arrays, and the renderer fetches the heavy detail for one message only when the user expands `Tools` or `Sources`. This keeps the transcript responsive under stress without dropping persisted provenance or changing the context assembly used for later LLM turns.

## The skill registry is DB-backed, while builtin skill files remain the seed source

Skills are now maintained in SQLite instead of only living as a process-local file scan. Built-in skills still ship as markdown under `skills/builtin`, but startup seeds them into the DB as read-only records; user-created skills are written directly into SQLite through the bridge. This keeps routing and capability search on a single authoritative registry, preserves local-first behavior, and lets the renderer manage skills through typed IPC without writing files directly.

Skill IDs are generated by the bridge from the user-provided title instead of being typed in the renderer. That keeps the DB as the authority, avoids renderer-side ID drift, and preserves stable skill references after creation because updates no longer rename IDs.
