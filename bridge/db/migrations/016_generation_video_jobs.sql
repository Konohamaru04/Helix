...
CREATE TABLE generation_jobs_backup AS
SELECT
  id,
  workspace_id,
  conversation_id,
...
CREATE TABLE generation_jobs (
  ...
  mode TEXT NOT NULL DEFAULT 'text-to-image'
    CHECK (mode IN ('text-to-image', 'image-to-image', 'image-to-video')),
  workflow_profile TEXT NOT NULL DEFAULT 'default'
    CHECK (workflow_profile IN ('default', 'qwen-image-edit-2511', 'wan-image-to-video'))
);
...
DROP TABLE generation_jobs_backup;
